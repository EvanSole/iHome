package com.website.common.security;
...
import com.website.common.util.EncodeUtils;
 ...
 * 
 *@deprecated  realm是shiro的桥梁，进行数据源配置，shrio提供了常用的realm数据源配置，
 * 如LDAP的JndiLdapRealm，JDBC的JdbcRealm，ini文件的IniRealm，properties文件的PropertiesRealm等，
 * 也可以插入自己的 Realm实现来代表自定义的数据源。此处使用了自定义的HomeRealm进行配置
 * 
 * @author Administrator
 * 
 */
public class HomeRealm extends AuthorizingRealm  {

	    static Logger logger = Logger.getLogger(HomeRealm.class);
	  ...
	  @Override
		protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
			...
			User user = userService.findUser(shiroUser.loginName);
			
			if (user != null) {
				  SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
				  //2)info.setRoles(角色集合);
				  List<Role> roleLists = roleService.selectList(user.getId());
				  for(Role role: roleLists){
                      info.addRole(role.getRoleName());
                      
					  //3)info.setStringPermissions(权限集合);
					  List<Permission>   permissionLists =  permissionService.selectList(role.getId());
